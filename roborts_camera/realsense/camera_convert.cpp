...
cv::Mat roborts_camera::camera_convert::get_depth_dst_new() {
    ...
    for (const auto &pixelPoint : pixel_point_colors_) {
        ...
        ptr[(int) (pixelPoint.pixel_points_in_color.y * (float) width_ + pixelPoint.pixel_points_in_color.x)]
...
cv::Mat roborts_camera::camera_convert::get_color_dst_() {
    ...
    for (const auto &pointColor : pixel_point_colors_) {
        ...
        ptr[(int) (pointColor.pixel_points_in_color.y * (float) width_ + pointColor.pixel_points_in_color.x)][0]
                = pointColor.pixel_points_rgb.x;
        ptr[(int) (pointColor.pixel_points_in_color.y * (float) width_ + pointColor.pixel_points_in_color.x)][1]
                = pointColor.pixel_points_rgb.y;
        ptr[(int) (pointColor.pixel_points_in_color.y * (float) width_ + pointColor.pixel_points_in_color.x)][2]
